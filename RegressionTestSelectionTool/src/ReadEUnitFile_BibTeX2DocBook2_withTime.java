...
public class ReadEUnitFile_BibTeX2DocBook2_withTime {
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			input = new Scanner(Paths.get("eunit/mytest_BibTeX2DocBook.eunit"));
		}catch(IOException e) {
		...
		try {
			input = new Scanner(Paths.get("ATLPrograms/BibTeX2DocBook.atl"));
		}catch(IOException e) {
			System.err.println("Error openning file.");
			System.exit(1);
		}
...
//			String sourceModel = StringUtils.replace(e.getValue().outputModel, "DocBook", "AutomaticModel_BibTex" );
...
//		}
	...
	public static void buildTestCaseModel(TestCaseSet tcSet, String model) throws IOException {
		...
		ResourceSet resourceSet = new ResourceSetImpl();
		resourceSet.getPackageRegistry().put(BibTeXPackage.eNS_URI, BibTeXPackage.eINSTANCE);
		resourceSet.getResourceFactoryRegistry().getExtensionToFactoryMap().put("*", new XMIResourceFactoryImpl());
		String loc = ReadEUnitFile_BibTeX2DocBook2_withTime.class.getResource("models/"+ model).getFile();
		Resource resource = resourceSet.createResource(URI.createFileURI(loc));
		...
		for(String c:listClasses)
			for(String r:lRules(c))
				for(Entry<String,Tuple> e:tc_i_o.entrySet()) {
					if(StringUtils.substringAfter(e.getValue().outputModel,"DocBook").equals(
							StringUtils.substringBetween(model,"AutomaticModel_BibTex",".xmi"))) {
						/* add rule */
	...
	public static ArrayList<String> lRules(String c){
		...
		for (Entry<String,String> entry : r_class.entrySet()) {  
			try {
				...
				String cClassName = "BibTeX.impl."+c+"Impl";
				String eClassName = "BibTeX.impl."+entry.getValue()+"Impl";
